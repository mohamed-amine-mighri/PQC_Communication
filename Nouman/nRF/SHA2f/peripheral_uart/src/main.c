...
static uint8_t SPHINCSSHA2128FSIMPLE_pk[PQCLEAN_SPHINCSSHA2128FSIMPLE_CLEAN_CRYPTO_PUBLICKEYBYTES];
static uint8_t SPHINCSSHA2128FSIMPLE_sk[PQCLEAN_SPHINCSSHA2128FSIMPLE_CLEAN_CRYPTO_SECRETKEYBYTES];
static uint8_t SPHINCSSHA2128FSIMPLE_signature[PQCLEAN_SPHINCSSHA2128FSIMPLE_CLEAN_CRYPTO_BYTES];
static size_t SPHINCSSHA2128FSIMPLE_signature_len;

static uint8_t SPHINCSSHA2128FSIMPLE_message[] = "hello from board_a";
static size_t SPHINCSSHA2128FSIMPLE_message_len = sizeof(SPHINCSSHA2128FSIMPLE_message) - 1;
...
static int send_public_key(void)
{
    ...
    const uint8_t *data = SPHINCSSHA2128FSIMPLE_pk;
    size_t total_len = PQCLEAN_SPHINCSSHA2128FSIMPLE_CLEAN_CRYPTO_PUBLICKEYBYTES;
...
static int send_signature(void)
{
    ...
    const uint8_t *data = SPHINCSSHA2128FSIMPLE_signature;
    size_t total_len = SPHINCSSHA2128FSIMPLE_signature_len;
...
static int send_message(void)
{
    ...
    const uint8_t *data = SPHINCSSHA2128FSIMPLE_message;
    size_t total_len = SPHINCSSHA2128FSIMPLE_message_len;
...
static void SPHINCSSHA2128FSIMPLE_keygen_thread(void *p1, void *p2, void *p3)
{
    ARG_UNUSED(p1);
    ARG_UNUSED(p2);
    ARG_UNUSED(p3);
  ...
  //  LOG_INF("Starting key generation...");
    ...
    int ret = PQCLEAN_SPHINCSSHA2128FSIMPLE_CLEAN_crypto_sign_keypair(
        SPHINCSSHA2128FSIMPLE_pk,
        SPHINCSSHA2128FSIMPLE_sk
    );
  ...
  //  LOG_INF("Public key size: %d bytes", PQCLEAN_SPHINCSSHA2128FSIMPLE_CLEAN_CRYPTO_PUBLICKEYBYTES);

 //   LOG_INF("Secret key size: %d bytes", PQCLEAN_SPHINCSSHA2128FSIMPLE_CLEAN_CRYPTO_SECRETKEYBYTES);


	/* Message to sign */
//	LOG_INF("Message: %s", SPHINCSSHA2128FSIMPLE_message);
//	LOG_INF("Message length: %d bytes", SPHINCSSHA2128FSIMPLE_message_len);
	...
	ret = PQCLEAN_SPHINCSSHA2128FSIMPLE_CLEAN_crypto_sign_signature(
		SPHINCSSHA2128FSIMPLE_signature,
		&SPHINCSSHA2128FSIMPLE_signature_len,
		SPHINCSSHA2128FSIMPLE_message,
		SPHINCSSHA2128FSIMPLE_message_len,
		SPHINCSSHA2128FSIMPLE_sk
	);
    ...
    if (ret == 0) {
        ...
        LOG_INF("Signing time: %lld ms",
                end_time - start_time);
        LOG_INF("Signature size: %d bytes", SPHINCSSHA2128FSIMPLE_signature_len);
    } else {
...
//	LOG_INF("Received data from: %s", addr);
	...
	if (len == 5 && memcmp(data, "START", 5) == 0) {
		...
		k_thread_create(&mldsa_thread_data,
		                ...
		                SPHINCSSHA2128FSIMPLE_keygen_thread,
		                NULL, NULL, NULL,
		                MLDSA_PRIORITY,
		                0,
		                K_NO_WAIT);
