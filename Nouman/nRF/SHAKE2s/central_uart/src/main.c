...
#define SPHINCSSHAKE128SSIMPLE_PUBLIC_KEY_SIZE PQCLEAN_SPHINCSSHAKE128SSIMPLE_CLEAN_CRYPTO_PUBLICKEYBYTES

static uint8_t public_key[SPHINCSSHAKE128SSIMPLE_PUBLIC_KEY_SIZE];
static uint16_t public_key_received = 0;
static bool public_key_complete = false;


#define SPHINCSSHAKE128SSIMPLE_SIGNATURE_SIZE PQCLEAN_SPHINCSSHAKE128SSIMPLE_CLEAN_CRYPTO_BYTES

static uint8_t signature[SPHINCSSHAKE128SSIMPLE_SIGNATURE_SIZE];
 ...
 */
#define SPHINCSSHAKE128SSIMPLE_MESSAGE_SIZE 18

static uint8_t message[SPHINCSSHAKE128SSIMPLE_MESSAGE_SIZE];
...
gpio_pin_set(gpio0, VERIFY_GPIO, 1);
	int verify_ret = PQCLEAN_SPHINCSSHAKE128SSIMPLE_CLEAN_crypto_sign_verify(
		signature,
		SPHINCSSHAKE128SSIMPLE_SIGNATURE_SIZE,
		message,
		message_received,
		public_key
	);
				 ...
				 uint16_t len)
{
	...
	if (!public_key_complete) {
		...
		if (public_key_received + len <=
		    SPHINCSSHAKE128SSIMPLE_PUBLIC_KEY_SIZE) {
		...
		//	LOG_INF("Public key received: %u / %u bytes",public_key_received,SPHINCSSHAKE128SSIMPLE_PUBLIC_KEY_SIZE);
	...
	if (!signature_complete) {
		...
		//	LOG_INF("Signature received: %u / %u bytes",signature_received,	SPHINCSSHAKE128SSIMPLE_SIGNATURE_SIZE);
	...
	if (!message_complete) {
		...
		if (message_received + len <=
		    SPHINCSSHAKE128SSIMPLE_MESSAGE_SIZE) {
			...
			//LOG_INF("Message received: %u / %u bytes",message_received,SPHINCSSHAKE128SSIMPLE_MESSAGE_SIZE);
