...
static uint8_t SPHINCSSHAKE128SSIMPLE_pk[PQCLEAN_SPHINCSSHAKE128SSIMPLE_CLEAN_CRYPTO_PUBLICKEYBYTES];
static uint8_t SPHINCSSHAKE128SSIMPLE_sk[PQCLEAN_SPHINCSSHAKE128SSIMPLE_CLEAN_CRYPTO_SECRETKEYBYTES];
static uint8_t SPHINCSSHAKE128SSIMPLE_signature[PQCLEAN_SPHINCSSHAKE128SSIMPLE_CLEAN_CRYPTO_BYTES];
static size_t SPHINCSSHAKE128SSIMPLE_signature_len;

static uint8_t SPHINCSSHAKE128SSIMPLE_message[] = "hello from board_a";
static size_t SPHINCSSHAKE128SSIMPLE_message_len = sizeof(SPHINCSSHAKE128SSIMPLE_message) - 1;
...
static int send_public_key(void)
{
    ...
    const uint8_t *data = SPHINCSSHAKE128SSIMPLE_pk;
    size_t total_len = PQCLEAN_SPHINCSSHAKE128SSIMPLE_CLEAN_CRYPTO_PUBLICKEYBYTES;
...
static int send_signature(void)
{
    ...
    const uint8_t *data = SPHINCSSHAKE128SSIMPLE_signature;
    size_t total_len = SPHINCSSHAKE128SSIMPLE_signature_len;
...
static int send_message(void)
{
    ...
    const uint8_t *data = SPHINCSSHAKE128SSIMPLE_message;
    size_t total_len = SPHINCSSHAKE128SSIMPLE_message_len;
...
static void SPHINCSSHAKE128SSIMPLE_keygen_thread(void *p1, void *p2, void *p3)
{
    ARG_UNUSED(p1);
    ARG_UNUSED(p2);
    ARG_UNUSED(p3);
  ...
  //  LOG_INF("Starting key generation...");
    ...
    int ret = PQCLEAN_SPHINCSSHAKE128SSIMPLE_CLEAN_crypto_sign_keypair(
        SPHINCSSHAKE128SSIMPLE_pk,
        SPHINCSSHAKE128SSIMPLE_sk
    );
  ...
  //  LOG_INF("Public key size: %d bytes", PQCLEAN_SPHINCSSHAKE128SSIMPLE_CLEAN_CRYPTO_PUBLICKEYBYTES);

 //   LOG_INF("Secret key size: %d bytes", PQCLEAN_SPHINCSSHAKE128SSIMPLE_CLEAN_CRYPTO_SECRETKEYBYTES);


	/* Message to sign */
//	LOG_INF("Message: %s", SPHINCSSHAKE128SSIMPLE_message);
//	LOG_INF("Message length: %d bytes", SPHINCSSHAKE128SSIMPLE_message_len);
	...
	ret = PQCLEAN_SPHINCSSHAKE128SSIMPLE_CLEAN_crypto_sign_signature(
		SPHINCSSHAKE128SSIMPLE_signature,
		&SPHINCSSHAKE128SSIMPLE_signature_len,
		SPHINCSSHAKE128SSIMPLE_message,
		SPHINCSSHAKE128SSIMPLE_message_len,
		SPHINCSSHAKE128SSIMPLE_sk
	);
    ...
    if (ret == 0) {
        ...
        LOG_INF("Signing time: %lld ms",
                end_time - start_time);
        LOG_INF("Signature size: %d bytes", SPHINCSSHAKE128SSIMPLE_signature_len);
    } else {
...
//	LOG_INF("Received data from: %s", addr);
	...
	if (len == 5 && memcmp(data, "START", 5) == 0) {
		...
		k_thread_create(&mldsa_thread_data,
		                ...
		                SPHINCSSHAKE128SSIMPLE_keygen_thread,
		                NULL, NULL, NULL,
		                MLDSA_PRIORITY,
		                0,
		                K_NO_WAIT);
