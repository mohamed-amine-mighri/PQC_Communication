...
static uint8_t FALCON512_pk[PQCLEAN_FALCON512_CLEAN_CRYPTO_PUBLICKEYBYTES];
static uint8_t FALCON512_sk[PQCLEAN_FALCON512_CLEAN_CRYPTO_SECRETKEYBYTES];
static uint8_t FALCON512_signature[PQCLEAN_FALCON512_CLEAN_CRYPTO_BYTES];
static size_t FALCON512_signature_len;

static uint8_t FALCON512_message[] = "hello from board_a";
static size_t FALCON512_message_len = sizeof(FALCON512_message) - 1;
...
/*
 * Sends the ML-DSA-44 (FALCON512) signature length as a single BLE NUS
 * notification, so the peer knows how many bytes to expect for the
 * signature. This transfer is NOT PPK2-triggered and is NOT timed —
 * it is purely a small metadata message sent right before the public
 * key transfer begins.
 */
static int send_signature_length(void)
{
    ...
    uint16_t sig_len = (uint16_t)FALCON512_signature_len;

    int ret = bt_nus_send(current_conn, (uint8_t *)&sig_len, sizeof(sig_len));

    if (ret) {
        LOG_ERR("Failed to send signature length, ret=%d", ret);
        return ret;
    }

    LOG_INF("Signature length sent: %u bytes", sig_len);
...
static int send_public_key(void)
{
    ...
    const uint8_t *data = FALCON512_pk;
    size_t total_len = PQCLEAN_FALCON512_CLEAN_CRYPTO_PUBLICKEYBYTES;
...
static int send_signature(void)
{
    ...
    const uint8_t *data = FALCON512_signature;
    size_t total_len = FALCON512_signature_len;
...
static int send_message(void)
{
    ...
    const uint8_t *data = FALCON512_message;
    size_t total_len = FALCON512_message_len;
...
static void FALCON512_keygen_thread(void *p1, void *p2, void *p3)
{
    ARG_UNUSED(p1);
    ARG_UNUSED(p2);
    ARG_UNUSED(p3);
  ...
  //  LOG_INF("Starting key generation...");
    ...
    int ret = PQCLEAN_FALCON512_CLEAN_crypto_sign_keypair(
        FALCON512_pk,
        FALCON512_sk
    );
  ...
  //  LOG_INF("Public key size: %d bytes", PQCLEAN_FALCON512_CLEAN_CRYPTO_PUBLICKEYBYTES);

 //   LOG_INF("Secret key size: %d bytes", PQCLEAN_FALCON512_CLEAN_CRYPTO_SECRETKEYBYTES);


	/* Message to sign */
//	LOG_INF("Message: %s", FALCON512_message);
//	LOG_INF("Message length: %d bytes", FALCON512_message_len);
	...
	ret = PQCLEAN_FALCON512_CLEAN_crypto_sign_signature(
		FALCON512_signature,
		&FALCON512_signature_len,
		FALCON512_message,
		FALCON512_message_len,
		FALCON512_sk
	);
    ...
    if (ret == 0) {
        ...
        LOG_INF("Signing time: %lld ms",
                end_time - start_time);
        LOG_INF("Signature size: %d bytes", FALCON512_signature_len);
    } else {
        LOG_ERR("signing FAILED, ret=%d", ret);
    }

	/* Send the signature length first (untimed, no PPK2 trigger),
	 * then start the public-key transfer.
	 */
	send_signature_length();
	send_public_key();
