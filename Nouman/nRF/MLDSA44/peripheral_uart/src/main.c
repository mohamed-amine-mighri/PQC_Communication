...
static uint8_t mldsa44_pk[PQCLEAN_MLDSA44_CLEAN_CRYPTO_PUBLICKEYBYTES];
static uint8_t mldsa44_sk[PQCLEAN_MLDSA44_CLEAN_CRYPTO_SECRETKEYBYTES];
static uint8_t mldsa44_signature[PQCLEAN_MLDSA44_CLEAN_CRYPTO_BYTES];
static size_t mldsa44_signature_len;

static uint8_t mldsa44_message[] = "hello from board_a";
static size_t mldsa44_message_len = sizeof(mldsa44_message) - 1;
...
static int send_public_key(void)
{
    ...
    const uint8_t *data = mldsa44_pk;
    size_t total_len = PQCLEAN_MLDSA44_CLEAN_CRYPTO_PUBLICKEYBYTES;
...
static int send_signature(void)
{
    ...
    const uint8_t *data = mldsa44_signature;
    size_t total_len = mldsa44_signature_len;
...
static int send_message(void)
{
    ...
    const uint8_t *data = mldsa44_message;
    size_t total_len = mldsa44_message_len;
...
static void mldsa44_keygen_thread(void *p1, void *p2, void *p3)
{
    ARG_UNUSED(p1);
    ARG_UNUSED(p2);
    ARG_UNUSED(p3);
  ...
  //  LOG_INF("Starting key generation...");
    ...
    int ret = PQCLEAN_MLDSA44_CLEAN_crypto_sign_keypair(
        mldsa44_pk,
        mldsa44_sk
    );
  ...
  //  LOG_INF("Public key size: %d bytes", PQCLEAN_MLDSA44_CLEAN_CRYPTO_PUBLICKEYBYTES);

 //   LOG_INF("Secret key size: %d bytes", PQCLEAN_MLDSA44_CLEAN_CRYPTO_SECRETKEYBYTES);


	/* Message to sign */
//	LOG_INF("Message: %s", mldsa44_message);
//	LOG_INF("Message length: %d bytes", mldsa44_message_len);
    ...
    if (ret == 0) {
        ...
        LOG_INF("Signing time: %lld ms",
                end_time - start_time);
     //   LOG_INF("Signature size: %d bytes", mldsa44_signature_len);
    } else {
