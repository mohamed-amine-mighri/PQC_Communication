...
static uint8_t public_key[MLDSA44_PUBLIC_KEY_SIZE];
...
static uint8_t signature[MLDSA44_SIGNATURE_SIZE];
 ...
 */
#define MLDSA44_MESSAGE_SIZE 18

static uint8_t message[MLDSA44_MESSAGE_SIZE];
 ...
 * PQC WORK QUEUE
 *
 * PQClean's clean ML-DSA-44 implementation stack-allocates
...
gpio_pin_set(gpio0, VERIFY_GPIO, 1);
	int verify_ret = PQCLEAN_MLDSA44_CLEAN_crypto_sign_verify(
		signature,
...
gpio_pin_set(gpio0, VERIFY_GPIO, 0);
	...
	if (verify_ret == 0) {
		LOG_INF("ML-DSA-44 SIGNATURE VERIFICATION SUCCESS");
	} else {
		LOG_ERR("ML-DSA-44 SIGNATURE VERIFICATION FAILED");
	}
				 ...
				 uint16_t len)
{
	...
	if (!public_key_complete) {
		...
		//	LOG_INF("Public key received: %u / %u bytes",public_key_received,MLDSA44_PUBLIC_KEY_SIZE);
			...
			if (public_key_received ==
				...
				//LOG_INF("================================");
				LOG_INF("ML-DSA-44 PUBLIC KEY RECEIVED");
	...
	if (!signature_complete) {
		...
		if (signature_received + len <=
			...
			//LOG_INF("Signature received: %u / %u bytes",signature_received,	MLDSA44_SIGNATURE_SIZE);
			...
			//	LOG_INF("================================");
				LOG_INF("ML-DSA-44 SIGNATURE RECEIVED");
	...
	if (!message_complete) {
		...
		if (message_received + len <=
			...
			//LOG_INF("Message received: %u / %u bytes",message_received,MLDSA44_MESSAGE_SIZE);
