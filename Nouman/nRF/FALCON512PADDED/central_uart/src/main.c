...
#define FALCONPADDED512_PUBLIC_KEY_SIZE PQCLEAN_FALCONPADDED512_CLEAN_CRYPTO_PUBLICKEYBYTES

static uint8_t public_key[FALCONPADDED512_PUBLIC_KEY_SIZE];
static uint16_t public_key_received = 0;
static bool public_key_complete = false;


#define FALCONPADDED512_SIGNATURE_SIZE PQCLEAN_FALCONPADDED512_CLEAN_CRYPTO_BYTES

static uint8_t signature[FALCONPADDED512_SIGNATURE_SIZE];
 ...
 */
#define FALCONPADDED512_MESSAGE_SIZE 18

static uint8_t message[FALCONPADDED512_MESSAGE_SIZE];
...
gpio_pin_set(gpio0, VERIFY_GPIO, 1);
	int verify_ret = PQCLEAN_FALCONPADDED512_CLEAN_crypto_sign_verify(
		signature,
		FALCONPADDED512_SIGNATURE_SIZE,
		message,
		message_received,
		public_key
	);
				 ...
				 uint16_t len)
{
	...
	if (!public_key_complete) {
		...
		if (public_key_received + len <=
		    FALCONPADDED512_PUBLIC_KEY_SIZE) {
		...
		//	LOG_INF("Public key received: %u / %u bytes",public_key_received,FALCONPADDED512_PUBLIC_KEY_SIZE);
	...
	if (!signature_complete) {
		...
		if (signature_received + len <=
			...
			LOG_INF("Signature received: %u / %u bytes",signature_received,	FALCONPADDED512_SIGNATURE_SIZE);
	...
	if (!message_complete) {
		...
		if (message_received + len <=
			...
			//LOG_INF("Message received: %u / %u bytes",message_received,FALCONPADDED512_MESSAGE_SIZE);
