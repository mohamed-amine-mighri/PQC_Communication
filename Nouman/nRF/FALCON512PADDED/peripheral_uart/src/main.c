...
static uint8_t FALCONPADDED512_pk[PQCLEAN_FALCONPADDED512_CLEAN_CRYPTO_PUBLICKEYBYTES];
static uint8_t FALCONPADDED512_sk[PQCLEAN_FALCONPADDED512_CLEAN_CRYPTO_SECRETKEYBYTES];
static uint8_t FALCONPADDED512_signature[PQCLEAN_FALCONPADDED512_CLEAN_CRYPTO_BYTES];
static size_t FALCONPADDED512_signature_len;

static uint8_t FALCONPADDED512_message[] = "hello from board_a";
static size_t FALCONPADDED512_message_len = sizeof(FALCONPADDED512_message) - 1;
...
static int send_public_key(void)
{
    ...
    const uint8_t *data = FALCONPADDED512_pk;
    size_t total_len = PQCLEAN_FALCONPADDED512_CLEAN_CRYPTO_PUBLICKEYBYTES;
...
static int send_signature(void)
{
    ...
    const uint8_t *data = FALCONPADDED512_signature;
    size_t total_len = FALCONPADDED512_signature_len;
...
static int send_message(void)
{
    ...
    const uint8_t *data = FALCONPADDED512_message;
    size_t total_len = FALCONPADDED512_message_len;
...
static void FALCONPADDED512_keygen_thread(void *p1, void *p2, void *p3)
{
    ARG_UNUSED(p1);
    ARG_UNUSED(p2);
    ARG_UNUSED(p3);
  ...
  //  LOG_INF("Starting key generation...");
    ...
    int ret = PQCLEAN_FALCONPADDED512_CLEAN_crypto_sign_keypair(
        FALCONPADDED512_pk,
        FALCONPADDED512_sk
    );
  ...
  //  LOG_INF("Public key size: %d bytes", PQCLEAN_FALCONPADDED512_CLEAN_CRYPTO_PUBLICKEYBYTES);

 //   LOG_INF("Secret key size: %d bytes", PQCLEAN_FALCONPADDED512_CLEAN_CRYPTO_SECRETKEYBYTES);


	/* Message to sign */
//	LOG_INF("Message: %s", FALCONPADDED512_message);
//	LOG_INF("Message length: %d bytes", FALCONPADDED512_message_len);
	...
	ret = PQCLEAN_FALCONPADDED512_CLEAN_crypto_sign_signature(
		FALCONPADDED512_signature,
		&FALCONPADDED512_signature_len,
		FALCONPADDED512_message,
		FALCONPADDED512_message_len,
		FALCONPADDED512_sk
	);
    ...
    if (ret == 0) {
        ...
        LOG_INF("Signing time: %lld ms",
                end_time - start_time);
        LOG_INF("Signature size: %d bytes", FALCONPADDED512_signature_len);
    } else {
