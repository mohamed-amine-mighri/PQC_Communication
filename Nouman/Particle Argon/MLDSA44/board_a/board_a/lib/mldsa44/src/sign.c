...
/*************************************************
 * Shared static scratch workspace.
 *
 * crypto_sign_keypair, crypto_sign_signature_ctx, and
 * crypto_sign_verify_ctx are never called concurrently on this
 * single-threaded device, so they share one static union instead
 * of each reserving separate large stack/BSS blocks. This avoids
 * both stack overflow (large polyvecl/polyveck arrays don't fit
 * on the thread stack) and BSS overflow (reserving all three
 * blocks separately exceeds available SRAM).
 *
 * NOT thread-safe / re-entrant. Do not call these functions from
 * multiple threads or nested/concurrently.
 **************************************************/
static union {
    struct {
        polyvecl mat[K];
...
#define mat    (ws.kp.mat)
#define s1     (ws.kp.s1)
#define s1hat  (ws.kp.s1hat)
#define s2     (ws.kp.s2)
#define t1     (ws.kp.t1)
#define t0     (ws.kp.t0)
