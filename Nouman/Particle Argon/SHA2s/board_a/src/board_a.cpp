...
// key buffers
uint8_t pk[PQCLEAN_SPHINCSSHA2128SSIMPLE_CLEAN_CRYPTO_PUBLICKEYBYTES];
uint8_t sk[PQCLEAN_SPHINCSSHA2128SSIMPLE_CLEAN_CRYPTO_SECRETKEYBYTES];

// Signature buffer
uint8_t sig[PQCLEAN_SPHINCSSHA2128SSIMPLE_CLEAN_CRYPTO_BYTES];
size_t sigLen = 0;
...
char pkHex[PQCLEAN_SPHINCSSHA2128SSIMPLE_CLEAN_CRYPTO_PUBLICKEYBYTES * 2 + 1];
char sigHex[PQCLEAN_SPHINCSSHA2128SSIMPLE_CLEAN_CRYPTO_BYTES * 2 + 1];
...
#define TOTAL_SIG_HEX_LEN (PQCLEAN_SPHINCSSHA2128SSIMPLE_CLEAN_CRYPTO_BYTES * 2)
...
void generateKeys() {
    ...
    int ret = PQCLEAN_SPHINCSSHA2128SSIMPLE_CLEAN_crypto_sign_keypair(pk, sk);
...
void signMessage() {
    ...
    int ret = PQCLEAN_SPHINCSSHA2128SSIMPLE_CLEAN_crypto_sign_signature(
        sig,
        &sigLen,
        message,
        messageLen,
...
void publishPublicKey() {
    ...
    bytesToHex(pk, PQCLEAN_SPHINCSSHA2128SSIMPLE_CLEAN_CRYPTO_PUBLICKEYBYTES, pkHex);
