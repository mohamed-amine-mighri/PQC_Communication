...
char pkHex[PQCLEAN_FALCONPADDED512_CLEAN_CRYPTO_PUBLICKEYBYTES * 2 + 1];
char sigHex[PQCLEAN_FALCONPADDED512_CLEAN_CRYPTO_BYTES * 2 + 1];
...
void generateKeys() {
    ...
    int ret = PQCLEAN_FALCONPADDED512_CLEAN_crypto_sign_keypair(pk, sk);
...
void signMessage() {
    ...
    int ret = PQCLEAN_FALCONPADDED512_CLEAN_crypto_sign_signature(
        sig,
        &sigLen,
        message,
        messageLen,
...
void publishPublicKey() {
    ...
    bytesToHex(pk, PQCLEAN_FALCONPADDED512_CLEAN_CRYPTO_PUBLICKEYBYTES, pkHex);
