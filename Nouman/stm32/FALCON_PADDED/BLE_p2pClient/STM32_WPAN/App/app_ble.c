...
/* USER CODE BEGIN PV */
static uint8_t server_msg_printed = 0;
/* USER CODE END PV */
    ...
    case HCI_VENDOR_SPECIFIC_DEBUG_EVT_CODE:
      {
        ...
        switch (blecore_evt->ecode)
        {
          ...
          case ACI_ATT_EXCHANGE_MTU_RESP_VSEVT_CODE:
            {
              aci_att_exchange_mtu_resp_event_rp0 *mtu_evt = (void*) blecore_evt->data;
              APP_DBG_MSG("-- ATT MTU NEGOTIATED: %d bytes (usable payload: %d bytes)\n\r",
                          mtu_evt->Server_RX_MTU, mtu_evt->Server_RX_MTU - 3);

              /**
               * The MTU exchange GATT procedure has now completed, so it is
               * safe to start the next GATT client procedure (service
               * discovery). Starting it any earlier collides with the
               * MTU exchange still in progress and is rejected with
    ...
    case HCI_LE_META_EVT_CODE:
      {
        ...
        switch (meta_evt->subevent)
        {
          ...
          case HCI_LE_CONNECTION_COMPLETE_SUBEVT_CODE:
            ...
            /* USER CODE BEGIN DLE_REQUEST */
            /**
             * Request Data Length Extension so the Link Layer uses larger
             * over-the-air packets (up to 251 bytes) instead of fragmenting
             * everything into legacy 27-byte packets.
             * connection_complete_event is valid here since it was just assigned above.
             * This is a controller-level HCI command, not a GATT client
             * procedure, so it does not collide with the GATT sequencing
             * below.
             */
            {
              tBleStatus dle_ret;
              dle_ret = hci_le_set_data_length(connection_complete_event->Connection_Handle, 251, 2120);
              if (dle_ret != BLE_STATUS_SUCCESS)
              {
                APP_DBG_MSG("  Fail   : hci_le_set_data_length command, result: 0x%x \n\r", dle_ret);
              }
              else
              {
                APP_DBG_MSG("  Success: hci_le_set_data_length command\n\r");
              }
            }
            /* USER CODE END DLE_REQUEST */
            ...
            /* USER CODE BEGIN MTU_EXCHANGE */
            /**
             * Request the ATT MTU exchange FIRST. Without this, both sides
             * stay at the legacy 23-byte MTU regardless of DLE /
             * CFG_BLE_MAX_ATT_MTU.
             *
             * IMPORTANT: aci_gatt_exchange_config() and
             * aci_gatt_disc_all_primary_services() are both asynchronous
             * GATT *client* procedures, and the stack only allows one
             * outstanding GATT client procedure per connection at a time.
             * Starting service discovery immediately after requesting the
             * MTU exchange (before the exchange has completed) gets
             * rejected with BLE_STATUS_FAILED / "Command Disallowed"
             * (0x0C). So we request the MTU exchange here, and only kick
             * off service discovery once ACI_ATT_EXCHANGE_MTU_RESP_VSEVT_CODE
             * confirms the exchange procedure has finished (see
             * Start_Service_Discovery() call in that event handler above).
             */
            {
              tBleStatus mtu_ret = aci_gatt_exchange_config(BleApplicationContext.BleApplicationContext_legacy.connectionHandle);
              if (mtu_ret != BLE_STATUS_SUCCESS)
              {
                APP_DBG_MSG("  Fail   : aci_gatt_exchange_config command, result: 0x%x \n\r", mtu_ret);
              }
              else
              {
                APP_DBG_MSG("  Success: aci_gatt_exchange_config command\n\r");
              }
            }
            /* USER CODE END MTU_EXCHANGE */

            /* NOTE: aci_gatt_disc_all_primary_services() is deliberately
             * NOT called here anymore. It is now triggered from
             * ACI_ATT_EXCHANGE_MTU_RESP_VSEVT_CODE via
             * Start_Service_Discovery(), once the MTU exchange procedure
                       ...
                       // APP_DBG_MSG("--- ST MANUFACTURER ID --- \n\r");
                        ...
                        {   /* Demo ID */
                           case CFG_DEV_ID_P2P_SERVER1: /* End Device 1 */
                        	   if (!server_msg_printed)
                        	   {
                        	       APP_DBG_MSG("-- SERVER DETECTED\n\r");
                        	       server_msg_printed = 1;
                        	   }
                          // APP_DBG_MSG("-- SERVER DETECTED \n\r");
                           ...
                           /* USER CODE BEGIN STOP_SCAN_EARLY */
                           {
                             tBleStatus term_ret = aci_gap_terminate_gap_proc(GAP_GENERAL_DISCOVERY_PROC);
                             if (term_ret != BLE_STATUS_SUCCESS)
                             {
                               APP_DBG_MSG("  Fail   : aci_gap_terminate_gap_proc command, result: 0x%x \n\r", term_ret);
                             }
                             else
                             {
                               APP_DBG_MSG("  Success: aci_gap_terminate_gap_proc command\n\r");
                             }
                           }
                           /* USER CODE END STOP_SCAN_EARLY */
          ...
          case HCI_LE_DATA_LENGTH_CHANGE_SUBEVT_CODE:
            {
              hci_le_data_length_change_event_rp0 *dle_evt = (hci_le_data_length_change_event_rp0*) meta_evt->data;
              APP_DBG_MSG("-- DLE UPDATED: MaxTxOctets=%d MaxRxOctets=%d\n\r",
                          dle_evt->MaxTxOctets, dle_evt->MaxRxOctets);
            }
            break;
...
/**
 * @brief  Starts GATT primary service discovery on the current connection.
 *         Must only be called once no other GATT client procedure is in
 *         progress on that connection (e.g. after the MTU exchange
 *         procedure has completed), otherwise the stack rejects the call
 *         with BLE_STATUS_FAILED (HCI status 0x0C, "Command Disallowed").
 */
static void Start_Service_Discovery(void)
{
  tBleStatus result;
  ...
  {
    APP_DBG_MSG("BLE_CTRL_App_Notification(), All services discovery Failed, result: 0x%x \r\n\r", result);
  }
}
