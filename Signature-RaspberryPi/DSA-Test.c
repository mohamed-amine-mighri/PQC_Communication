...
//    OQS_SIG_alg_ml_dsa_44,
   ...
   // OQS_SIG_alg_ml_dsa_87,
    // SPHINCS algorithms commented out - may not be available depending on liboqs build
...
bool test_dsa_Alice(char* algo){
    ...
    if(crypto_sign_message( signed_message, &actual_sm_size, (const uint8_t*)message, strlen(message), sk, algorithm) != 0) {
    ...
    // Wait for message
    bool isMessageReceived = false;    Message_struct* msg;
    while(!isMessageReceived){
        pthread_mutex_lock(&queue_lock);
        if(!isEmpty(&receiving_queue)){
            isMessageReceived = true;
...
void test_dsa_Bob(char* algo){
    ...
    algorithm = OQS_SIG_new(algo);
    if(!algorithm){
        printf("Failed to create algorithm\n");
        return;
    }
...
int main() {
    #if defined(ROLE_ALICE)
        transport_set_role(true);
    #else
        transport_set_role(false);
    #endif
