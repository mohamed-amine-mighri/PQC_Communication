#include "transport.h"
#include <string.h>
#include <stdlib.h>
#include <errno.h>
#include "MQTTClient.h"

// Conditionally include wiringPi (only available on Raspberry Pi, not on WSL)
#ifdef __arm__
    #include <wiringPi.h>
    #include <wiringSerial.h>
    #define WIRING_PI_AVAILABLE 1
#else
    #define WIRING_PI_AVAILABLE 0
#endif

#define Serial_1_MQTT_0 0

#if Serial_1_MQTT_0
    #define Serial
#else
    #define MQTT
#endif

#define ALICE_TOPIC "dsa/alice/send"
#define BOB_TOPIC "dsa/bob/send"

bool end = false;
Queue receiving_queue;
pthread_mutex_t queue_lock;
pthread_t receive_thread;
static bool transport_is_alice = false;

// ------ Serial -----

#define SERIAL_DEV "/dev/serial0"
#define BAUD_RATE 115200

int fd;

void transport_set_role(bool is_alice) {
    transport_is_alice = is_alice;
}

// Setup

int serial_setup() {
    #if WIRING_PI_AVAILABLE
        int serial_port;
        if((serial_port = serialOpen(SERIAL_DEV, BAUD_RATE)) < 0) {
            perror("SerialOpen failed");
            return -1;
        }

        if (wiringPiSetup() == -1) {
            perror("wiringPi Setup failed");
            return -1;
        }
        return serial_port;
    #else
        fprintf(stderr, "Serial not available on this platform\n");
        return -1;
    #endif
}

// Send

int serial_send(int fd, const uint8_t *data, size_t len) {
    #if WIRING_PI_AVAILABLE
        uint8_t hdr[2] = { (len >> 8) & 0xFF, len & 0xFF};
        serialPutchar(fd, hdr[0]);
        serialPutchar(fd, hdr[1]);
        for(size_t i = 0; i < len; i++) {
            serialPutchar(fd, data[i]);
        }
        return 0;
    #else
        fprintf(stderr, "Serial not available on this platform\n");
        return -1;
    #endif
}

// Read

int serial_read_exact(int fd, uint8_t *buf, size_t len) {
    #if WIRING_PI_AVAILABLE
        size_t got = 0;
        while (got < len && !end) {
            if(serialDataAvail(fd)) {
                int character = serialGetchar(fd);
                if(character < 0 ) break;
                buf[got++] = (uint8_t) character;
            }
        }
        return (got == len) ? got : -1;
    #else
        fprintf(stderr, "Serial not available on this platform\n");
        return -1;
    #endif
}

// ------ MQTT ------

// Setup
#define CLIENTID "RaspberryPiClient"

#define ADDRESS_MQTT "mqtt://10.42.0.1:1883"
#define RECEIVING_TOPIC "dsa/alice/send"
#define SENDING_TOPIC "dsa/bob/send"

#define MAX_BUFFER  65536
#define TIMEOUT 100L

unsigned char readBuffer[MAX_BUFFER];
size_t bufferLen = 0;
MQTTClient client;


void mqtt_setup() {
    MQTTClient_connectOptions conn_options = MQTTClient_connectOptions_initializer;
    int rc;

    const char* clientId = transport_is_alice ? "AliceClient" : "BobClient";
    MQTTClient_create(&client, ADDRESS_MQTT, clientId, MQTTCLIENT_PERSISTENCE_NONE, NULL);

    conn_options.keepAliveInterval = 20;
    conn_options.cleansession = 1;

    if((rc = MQTTClient_connect(client, &conn_options)) != MQTTCLIENT_SUCCESS) {
        printf("Failed to connect, retur code %d\n", rc);
        return;
    }

    MQTTClient_subscribe(client, ALICE_TOPIC, 0);
    MQTTClient_subscribe(client, BOB_TOPIC, 0);

    printf("connected to broker\n");
}

// Send

int mqtt_send(const uint8_t *data, size_t len){
    const char* topic = transport_is_alice ? BOB_TOPIC : ALICE_TOPIC;
    uint8_t hdr[2] = { (len >> 8) & 0xFF, len & 0xFF};
    if(MQTTClient_publish(client, topic, 2, hdr, 0, 0, NULL) != MQTTCLIENT_SUCCESS) {
        return -1;
    }
    return MQTTClient_publish(client, topic, len, data, 0, 0, NULL) == MQTTCLIENT_SUCCESS ? 0 : -1;
}
// Read

int mqtt_read(uint8_t *data, size_t len){
    int got = 0;
    char *topicName = NULL;
    int topicLen;
    MQTTClient_message *message = NULL;

    while(got < len && !end){
        if(bufferLen > 0){
            int to_copy = (len - got < bufferLen) ? (len - got) : bufferLen;
            bufferLen = bufferLen - to_copy;
            memcpy(data + got, readBuffer, to_copy);

            got += to_copy;

            if(bufferLen > 0){
                memcpy(readBuffer, readBuffer + to_copy, bufferLen);
            }
        } else if (MQTTClient_receive(client, &topicName, &topicLen, &message, TIMEOUT) == MQTTCLIENT_SUCCESS){
            if(message){
               if(strcmp(topicName, transport_is_alice ? BOB_TOPIC : ALICE_TOPIC) == 0) {
                    MQTTClient_freeMessage(&message);
                    MQTTClient_free(topicName);
                    continue;
                }
               printf("[MQTT RECV] topic=%s len=%d\n" , topicName, message->payloadlen);
                memcpy(readBuffer + bufferLen, message->payload, message->payloadlen);
                bufferLen += message->payloadlen;
                MQTTClient_freeMessage(&message);
                MQTTClient_free(topicName);
            }

        }

    }
    return (got == len) ? got : -1;
}

// destroy

void mqtt_destroy() {
    MQTTClient_disconnect(client, 10000);
    MQTTClient_destroy(&client);
    return;
}

//  ------ Wrapper ------

int setup() {
    initializeQueue(&receiving_queue);
    if(pthread_mutex_init(&queue_lock, NULL) != 0) {
        perror("pthread_mutex_init");
        return -1;
    }

    #if defined(Serial)
        fd = serial_setup();
        if(fd < 0) {
            printf("Unable to setup\n");
            return -1;
        }
    #else
        mqtt_setup();
    #endif

    if(pthread_create(&receive_thread, NULL, receive_task, NULL) != 0) {
        perror("pthread_create");
        return -1;
    }
    return 0;
}

int send_message(const uint8_t* data, size_t len) {
    #if defined(Serial)
        return serial_send(fd, data, len);
    #else 
        return mqtt_send(data, len);
    #endif
}

int receive_message(uint8_t* data, size_t len) {
    #if defined(Serial)
        return serial_read_exact(fd, data, len);
    #else
        return mqtt_read(data, len);
    #endif
}

void* receive_task(void* arg) {
    while(!end) {

        uint8_t hdr[2];
        if(receive_message(hdr, 2) < 0) {
            if(!end)  printf("Unable to get hdr\n");
            continue;
        }

        uint16_t len = (hdr[0] << 8) | hdr[1];

        uint8_t* data = malloc(len);
        if (!data) {
            printf("Unable to allocate space for data\n");
            continue;
        }

        if(receive_message(data, len) < 0) {
            printf("Unable to read data\n");
            free(data);
            continue;
        }

        Message_struct* msg = malloc(sizeof(Message_struct));
        if (!msg) {
            printf("Unable to allocate Message_struct\n");
            free(data);
            continue;
        }
        msg->content = data;
        msg->size = len;

        pthread_mutex_lock(&queue_lock);
        if(enqueue(&receiving_queue, msg) < 0) {
            pthread_mutex_unlock(&queue_lock);
            printf("Unable to enqueue\n");
            free(msg->content);
            free(msg);
            continue;
        }
        pthread_mutex_unlock(&queue_lock);
    }
    return NULL;
}

void destroy() {
    end = true;
    pthread_join(receive_thread, NULL);
    pthread_mutex_destroy(&queue_lock);
    #if defined(Serial)
        return; // Do nothing further
    #else 
        mqtt_destroy();
    #endif
}
